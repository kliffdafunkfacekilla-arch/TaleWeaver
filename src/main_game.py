...
def load_map_data():
    """Reads the JSON map state, extracting local map AND injecting meta for UI compatibility."""
    state_path = "state/local_map_state.json"
    for _ in range(10):
        try:
            with open(state_path, "r") as f:
                state = json.load(f)
                # Align with new root-meta architecture
                if "local_map_state" in state:
                    map_data = state["local_map_state"]
                    # Inject root levels into map_data for backward compatibility in UI code
                    map_data["meta"] = state.get("meta", {})
                    map_data["combat_log"] = state.get("combat_log", [])
                    return map_data
                return state
...
def draw_tactical_screen(map_data, cam_x, cam_y):
    ...
    if deck or campaign.get("active_subplot"):
        current_objective = campaign.get("active_subplot", "Explore the World")
...
def main():
    global status_text, app_state, transition_target, map_data # Added map_data to global
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT: pygame.quit(); sys.exit()
                    ...
                    if event.button == 1:
                            ...
                            if mr.collidepoint(mx, my):
                                if my > context_menu["y"] + hh:
                                    ...
                                    if idx < len(context_menu["options"]):
                                        ...
                                        if sel == "More Actions...": context_menu["page"] = "more"; target = next((e for e in map_data["entities"] if e.get("id") == context_menu["target_id"]), None); context_menu["options"] = generate_menu_options(target, player, map_data, "more"); continue
                                        elif sel == "Back": context_menu["page"] = "more"; target = next((e for e in map_data["entities"] if e.get("id") == context_menu["target_id"]), None); context_menu["options"] = generate_menu_options(target, player, map_data, "main"); continue
                                        if sel != "Cancel":
                        ...
                        if my < WINDOW_HEIGHT - UI_HEIGHT: context_menu["active"] = True; context_menu["x"], context_menu["y"] = mx, my; context_menu["page"] = "main"; context_menu["target_pos"] = [gx, gy]; context_menu["target_name"] = clicked_entity["name"] if clicked_entity else None; context_menu["target_id"] = clicked_entity.get("id", clicked_entity["name"]) if clicked_entity else None; context_menu["options"] = generate_menu_options(clicked_entity, player, map_data, "main")
            pygame.display.flip()
